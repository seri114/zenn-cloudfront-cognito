...
import configFromJson from './configuration.json';
...
export interface CompleteConfig extends ConfigWithHeaders, ConfigFromDiskComplete {
  cloudFrontHeaders: CloudFrontHeaders;
  secretAllowedCharacters: string;
  pkceLength: number;
  nonceLength: number;
...
export function getConfig(): Config {
  ...
  // ) as ConfigFromDisk;
  const config = configFromJson as ConfigFromDisk;
  return {
...
export interface GenerateCookieHeadersParam {
  clientId: string;
  oauthScopes: string[];
  cookieSettings: CookieSettings;
  cookieCompatibility: "amplify" | "elasticsearch";
...
export interface SignInCookieHeadersParam extends GenerateCookieHeadersParam {
  tokens: { id: string; access: string; refresh: string }
}

export interface RefreshCookieHeadersParam extends GenerateCookieHeadersParam {
  tokens: { id: string; access: string }
}


export const generateCookieHeaders = {
  signIn: (param: SignInCookieHeadersParam) => _generateCookieHeaders({ ...param, event: "signIn" }),
  refresh: (param: RefreshCookieHeadersParam) => _generateCookieHeaders({ ...param, event: "refresh" }),
  signOut: (param: GenerateCookieHeadersParam) => _generateCookieHeaders({ ...param, event: "signOut" }),
};

interface GenerateCookieHeadersEventParam extends GenerateCookieHeadersParam {
  event: "signIn" | "signOut" | "refresh"
}
type GeneratedCookies = {key: string, value: string}[]
function _generateCookieHeaders(param: GenerateCookieHeadersEventParam): GeneratedCookies {
  /**
  ...
  if (param.cookieCompatibility === "amplify") {
    ...
    Object.assign(cookies, {
      ...
      [cookieNames.userDataKey]: `${encodeURIComponent(userData)}; ${param.cookieSettings.idToken}`,
      "amplify-signin-with-hostedUI": `true; ${param.cookieSettings.accessToken}`,
    });
  } else {
    cookieNames = getElasticsearchCookieNames();
    cookies[cookieNames.cognitoEnabledKey] = `True; ${param.cookieSettings.cognitoEnabled}`;
  }
...
// !! Added !!
export const splitDomainName = (domainName: string): string => {
