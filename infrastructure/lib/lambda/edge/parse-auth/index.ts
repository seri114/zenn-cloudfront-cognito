...
import {splitDomainName} from "../shared/shared";
...
export const handler: CloudFrontRequestHandler = async (event) => {
  ...
  try {
    ...
    const requestConfig: Parameters<typeof common.httpPostToCognitoWithRetry>[2] = {
      headers: {
    ...
    const signInCookieHeadersParam: common.SignInCookieHeadersParam = {
      tokens: {
        id: idToken,
        access: accessToken,
    ...
    const setCookies = common.generateCookieHeaders.signIn(signInCookieHeadersParam)
    ...
    if (idTokenInCookies) {
      ...
      CONFIG.logger.debug("ID token found, redirecting back to:", redirectedFromUri);
