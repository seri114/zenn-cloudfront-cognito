...
export const handler: CloudFrontRequestHandler = async (event) => {
  ...
  try {
    ...
    const refreshCookieHeadersParam: common.RefreshCookieHeadersParam = {
      tokens: {
        id: newIdToken,
    ...
    const setCookies = common.generateCookieHeaders.refresh(refreshCookieHeadersParam)
    ...
    const response = {
      ...
      headers: {
        location: [
          {
            key: "location",
            value: `https://${domainName}${typeof requestedUri === "string" ? requestedUri : "/"}`,
          },
        ],
